...
from src.nlp.matching.utils import load_jsonl
from src.nlp.matching.build_features import ClinicalTrialsFeatures
from src.nlp.matching.index_clinical_trials import Indexer
from src.nlp.evaluation import read_bm25, evaluate

feature_builder = ClinicalTrialsFeatures(spacy_language_model_name="en_core_sci_sm") # TODO change to lg
...
if __name__ == "__main__":
    ...
    runs_dict = { # different options for querying and indexing!
    ...
    for run_name, options in runs_dict.items():
        ...
        logger.debug("Index expansion complete")

        logger.debug("Indexing %s", run_name)
        ...
        logger.debug("Querying %s", run_name)
        output_scores = {}
        ...
        with open(f"{args.output_dir}/bm25p-{run_name}-{TODAY}.json", "w") as fp:
            json.dump(output_scores, fp)

        run_output_file = f"{args.output_dir}/bm25p-{run_name}-{TODAY}"
        run_results_file = f"{args.output_dir}/bm25p-{run_name}-{TODAY}-results"
        ...
        logger.debug("Evaluating %s", run_name)
